...
# def draw_bev(detections, tracks, frame_idx, save_dir='./output/'):
...
#         x, y, z, l, w, h, yaw = det
#         rect = get_rotated_rect(x, y, l, w, yaw, color='blue', alpha=0.3)
#         ax.add_patch(rect)

#     for track in tracks:
#         x, y, z, l, w, h, yaw = track.bbox
#         color = get_color(track.id)
#         rect = get_rotated_rect(x, y, l, w, yaw, color=color, alpha=0.6)
#         ax.add_patch(rect)
#         ax.text(x, y, str(track.id), color=color, fontsize=8)

#     plt.grid(True)
#     plt.savefig(f"{save_dir}/frame_{frame_idx:04d}.png")
#     plt.close()

# def get_rotated_rect(x, y, length, width, yaw, color='red', alpha=0.5):
...
#         alpha=alpha
#     )
#     t = Affine2D().rotate_deg_around(0, 0, yaw * 180 / 3.14159).translate(x, y)
